...
const OTPInputCard: React.FC<OTPInputCardProps> = ({
  ...
  const handleSubmit = async (otp: string) => {
    try {
      axios.post("/api/auth/verify-otp", { email: email, otp: otp });
  ...
  return (
    <Card className="w-[380px]">
          ...
          <div className="flex items-center justify-between w-full text-sm text-neutral-600">
            ...
            <Button
              ...
              className={`justify-end p-0 ${
                remainingTime > 0 ? "text-neutral-400" : "text-primary"
              }`}
              onClick={() => setRemainingTime(60)}
            >
              {remainingTime > 0 ? `Resend in ${remainingTime}s` : "Resend OTP"}
            </Button>
          </div>
        </CardDescription>
      </CardHeader>
      ...
      <CardFooter className="flex justify-between gap-4">
        ...
        <Button
          onClick={() => {
            setOpen(false), handleSubmit(otp);
          }}
          className="w-full"
        >
          Confirm
